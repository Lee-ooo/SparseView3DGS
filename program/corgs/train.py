...
from utils.binocular_utils import make_shifted_camera, binocular_consistency_loss
...
def _depth_tensor(depth, reference):
    """Convert a numpy/tensor depth map to the rendered map's shape."""
    if not torch.is_tensor(depth):
        depth = torch.as_tensor(depth, device=reference.device, dtype=reference.dtype)
    else:
        depth = depth.to(device=reference.device, dtype=reference.dtype)
    if depth.ndim == 3:
        depth = depth[0]
    if depth.shape != reference.shape:
        depth = F.interpolate(
            depth[None, None], size=reference.shape[-2:], mode="bilinear", align_corners=False
        )[0, 0]
    return depth


def fsgs_training_depth_loss(rendered_depth, target_depth):
    """FSGS scale-invariant depth loss for a real training view."""
    rendered = rendered_depth[0] if rendered_depth.ndim == 3 else rendered_depth
    target = _depth_tensor(target_depth, rendered)
    rendered = torch.nan_to_num(rendered).reshape(-1, 1)
    target = torch.nan_to_num(target).reshape(-1, 1)
    loss = torch.minimum(
        1 - pearson_corrcoef(-target, rendered),
        1 - pearson_corrcoef(1 / (target + 200.0), rendered),
    )
    return torch.nan_to_num(loss, nan=0.0, posinf=0.0, neginf=0.0)


def fsgs_pseudo_depth_loss(rendered_depth, pseudo_image):
    """FSGS pseudo-view depth loss with gradients through the rendered RGB."""
    rendered = rendered_depth[0] if rendered_depth.ndim == 3 else rendered_depth
    estimated = estimate_depth(pseudo_image, mode="train")
    estimated = _depth_tensor(estimated, rendered)
    loss = 1 - pearson_corrcoef(rendered.reshape(-1, 1), -estimated.reshape(-1, 1))
    return torch.nan_to_num(loss, nan=0.0, posinf=0.0, neginf=0.0)


def pop_pseudo_camera(stack, pool):
    """Take one pseudo camera without failing on an empty pool.

    Generic COLMAP scenes can legitimately produce no valid ellipse poses
    (for example, when there are too few or degenerate camera centers). In
    that case the pseudo-view losses are skipped while real-view, binocular,
    and CoR-GS losses continue to train the models.
    """
    if not stack:
        stack = list(pool)
    if not stack:
        return None, stack
    return stack.pop(randint(0, len(stack) - 1)), stack


def save_pause_checkpoint(model_path, iteration, gs_dict, args, opt, reason):
    """Atomically save a resumable snapshot of all Gaussian fields.

    Unlike the original CoR-GS checkpoint, this snapshot contains every
    Gaussian field and its optimizer state, so it can be used to continue a
    multi-field run after a pause or an interrupted training process.
    """
    checkpoint_path = args.pause_checkpoint_path or os.path.join(model_path, "pause_latest.pth")
    checkpoint_path = os.path.abspath(checkpoint_path)
    os.makedirs(os.path.dirname(checkpoint_path), exist_ok=True)
    payload = {
        "format": "corgs_pause_v1",
        "iteration": int(iteration),
        "reason": str(reason),
        "models": {
            name: model.capture()
            for name, model in gs_dict.items()
            if hasattr(model, "capture")
        },
        "args": vars(args).copy(),
        "optimization": vars(opt).copy(),
    }
    temporary_path = checkpoint_path + ".tmp"
    with open(temporary_path, "wb") as checkpoint_file:
        torch.save(payload, checkpoint_file)
    os.replace(temporary_path, checkpoint_path)
    print(f"\n[PAUSE] Saved resumable checkpoint at iteration {iteration}: {checkpoint_path}")
...
def training(dataset, opt, pipe, args):
    ...
    if checkpoint:
        loaded_checkpoint = torch.load(checkpoint, map_location="cuda", weights_only=False)
        if isinstance(loaded_checkpoint, dict) and loaded_checkpoint.get("format") == "corgs_pause_v1":
            pause_payload = loaded_checkpoint
            first_iter = int(loaded_checkpoint["iteration"])
            print(f"Resuming from pause checkpoint at iteration {first_iter}")
        else:
    ...
    for i in range(args.gaussiansN):
        if i == 0:
            ...
            # A legacy CoR-GS run stores the second field as a PLY rather than
            # in the original single-field checkpoint.  Loading it here makes
            # it possible to continue a completed two-field run faithfully.
            if args.checkpoint2 and args.checkpoint2.lower().endswith(".ply"):
                GsDict[f"gs{i}"].load_ply(args.checkpoint2)
            else:
    ...
    if pause_payload is not None:
        saved_models = pause_payload.get("models", {})
        for name, model in GsDict.items():
            if name not in saved_models:
                raise ValueError(f"Pause checkpoint is missing Gaussian field: {name}")
            model.restore(saved_models[name], opt)
    ...
    progress_bar = tqdm(
        total=max(int(opt.iterations) - progress_start, 0),
        initial=0,
        desc="CoR-GS training",
        unit="iter",
    )

    viewpoint_stack, pseudo_stack = None, None
    pseudo_stack_co = None
    pseudo_stack_fsgs = None
    pseudo_camera_pool = [
        cam for cam in scene.getPseudoCameras().copy() if cam is not None
    ]
    print(f"Pseudo cameras available: {len(pseudo_camera_pool)}")
    ...
    exception_state = {
        "enabled": True,
        "saving": False,
        "iteration": max(first_iter - 1, 0),
    }

    def request_pause(_signal_number, _frame):
        if not pause_state["requested"]:
            pause_state["requested"] = True
            print("\n[PAUSE] Ctrl+C received; finishing this iteration and saving a checkpoint...")

    def save_on_exception(exception_type, exception_value, exception_traceback):
        if exception_state["enabled"] and not exception_state["saving"]:
            exception_state["saving"] = True
            try:
                save_pause_checkpoint(
                    scene.model_path,
                    exception_state["iteration"],
                    GsDict,
                    args,
                    opt,
                ...
                print(f"[PAUSE] Could not save emergency checkpoint: {save_error}")
        previous_exception_hook(exception_type, exception_value, exception_traceback)

    signal.signal(signal.SIGINT, request_pause)
    sys.excepthook = save_on_exception
    
    for iteration in range(first_iter, opt.iterations + 1):
        # If this iteration raises, the model is still consistent with the
        # previous completed iteration and can be resumed from that point.
        exception_state["iteration"] = max(iteration - 1, 0)
        ...
        # Optional FSGS depth regularization on the real training view.  The
        # depth image is computed once per camera by corgs/utils/camera_utils.py
        # when --fsgs_depth is enabled.
        fsgs_depth = getattr(args, "fsgs_depth", False)
        target_depth = getattr(viewpoint_cam, "depth_image", None)
        if fsgs_depth and target_depth is not None:
            depth_weight = getattr(args, "fsgs_depth_weight", 0.05)
            if iteration > getattr(args, "fsgs_depth_end", 9500):
                depth_weight = getattr(args, "fsgs_depth_final_weight", 0.001)
            for i in range(args.gaussiansN):
                LossDict[f"loss_gs{i}"] += depth_weight * fsgs_training_depth_loss(
                    RenderDict[f"depth_gs{i}"], target_depth
                )

        # Binocular3DGS-style self-supervision.  The virtual right camera is
        # generated only after the ordinary view has become reasonably stable.
        binocular_active = (
            args.binocular
            and iteration >= args.binocular_start
            and iteration % args.binocular_interval == 0
        )
        if binocular_active:
            baseline_fraction = random.uniform(
                args.binocular_baseline_min, args.binocular_baseline_max
            )
            # COLMAP has arbitrary global scale. Express the baseline as a
            # fraction of the camera trajectory radius so the same setting
            # remains meaningful across different SfM reconstructions.
            baseline = baseline_fraction * max(float(scene.cameras_extent), 1e-4)
            baseline *= random.choice((-1.0, 1.0))
            binocular_cam = make_shifted_camera(viewpoint_cam, baseline)
            for i in range(args.gaussiansN):
                right_pkg = render(binocular_cam, GsDict[f"gs{i}"], pipe, bg)
                stereo_loss, stereo_valid = binocular_consistency_loss(
                    gt_image,
                    RenderDict[f"render_pkg_gs{i}"],
                    right_pkg,
                    viewpoint_cam,
                    baseline,
                    lambda image, target, valid=None: loss_photometric(
                        image, target, opt=opt, valid=valid
                    ),
                    min_alpha=args.binocular_min_alpha,
                )
                LossDict[f"loss_gs{i}"] += args.binocular_weight * stereo_loss
                # Do not keep the virtual-view render graph in RenderDict;
                # it is only needed through the current backward pass.
                del right_pkg, stereo_valid
            del binocular_cam, stereo_loss

        # FSGS pseudo-view depth regularization.  This is deliberately kept
        # separate from CoR-GS pseudo-view RGB co-regularization so both losses
        # can be active in the combined mode.
        if (
            getattr(args, "fsgs_depth_pseudo", False)
            and iteration % getattr(args, "fsgs_depth_pseudo_interval", 10) == 0
            and iteration > getattr(args, "fsgs_depth_start", 2000)
            and iteration < getattr(args, "fsgs_depth_end", 9500)
        ):
            pseudo_cam_fsgs, pseudo_stack_fsgs = pop_pseudo_camera(
                pseudo_stack_fsgs, pseudo_camera_pool
            )
            if pseudo_cam_fsgs is not None:
                pseudo_loss_scale = min(
                    (iteration - getattr(args, "fsgs_depth_start", 2000)) / 500.0, 1.0
                )
                for i in range(args.gaussiansN):
                    pseudo_pkg = render(pseudo_cam_fsgs, GsDict[f"gs{i}"], pipe, bg)
                    pseudo_depth_loss = fsgs_pseudo_depth_loss(
                        pseudo_pkg["depth"], pseudo_pkg["render"]
                    )
                    LossDict[f"loss_gs{i}"] += (
                        pseudo_loss_scale
                        * getattr(args, "fsgs_depth_pseudo_weight", 0.5)
                        * pseudo_depth_loss
                    )
                    del pseudo_pkg
                del pseudo_cam_fsgs
        ...
        if not args.onlyrgb:
            if (iteration % args.sample_pseudo_interval == 0
                ...
                pseudo_cam_co, pseudo_stack_co = pop_pseudo_camera(
                    pseudo_stack_co, pseudo_camera_pool
                )
                if pseudo_cam_co is not None:
                    for i in range(args.gaussiansN):
        ...
        with torch.no_grad():
            ...
            if iteration % 10 == 0 or iteration == opt.iterations:
                ...
                progress_bar.update(max(iteration - progress_start - progress_bar.n, 0))
            if iteration == opt.iterations:
                progress_bar.close()

            # Save the model before optional metrics.  LPIPS may need an
            # unavailable VGG download and must not prevent checkpoint output.
            if iteration >= first_iter and (iteration in saving_iterations):
                print("\n[ITER {}] Saving Gaussians".format(iteration))
                scene.save(iteration)
                if args.gaussiansN == 2 and args.save_second_gaussian:
            ...
            if (args.coprune and args.coprune_interval > 0
                    and iteration > opt.densify_from_iter
                    and iteration < opt.iterations
                    and iteration % args.coprune_interval == 0):
                for i in range(args.gaussiansN):
                    for j in range(args.gaussiansN):
                        if i != j:
            ...
            if (args.pause_checkpoint_interval > 0
                    and iteration % args.pause_checkpoint_interval == 0):
                save_pause_checkpoint(scene.model_path, iteration, GsDict, args, opt, "periodic")

            if pause_state["requested"]:
                save_pause_checkpoint(scene.model_path, iteration, GsDict, args, opt, "user_pause")
                progress_bar.close()
                break

            exception_state["iteration"] = iteration

    signal.signal(signal.SIGINT, previous_sigint_handler)
    exception_state["enabled"] = False
...
if __name__ == "__main__":
    ...
    parser.add_argument('--save_second_gaussian', action='store_true', default=False,
                        help='Persist the second CoR-GS field as point_cloud_gs2; off by default.')

    parser.add_argument("--binocular", action="store_true", default=False)
    parser.add_argument("--binocular_start", type=int, default=2000)
    parser.add_argument("--binocular_interval", type=int, default=10)
    parser.add_argument("--binocular_weight", type=float, default=0.2)
    parser.add_argument("--binocular_baseline_min", type=float, default=0.01)
    parser.add_argument("--binocular_baseline_max", type=float, default=0.08)
    parser.add_argument("--binocular_min_alpha", type=float, default=0.05)
    ...
    parser.add_argument('--coprune_interval', type=int, default=500)
    parser.add_argument('--pause_checkpoint_interval', type=int, default=500,
                        help='Save a resumable pause_latest.pth every N iterations; 0 disables periodic saves.')
    parser.add_argument('--pause_checkpoint_path', type=str, default="",
                        help='Optional path for the resumable pause checkpoint.')

    parser.add_argument("--save_log_images", action="store_true")

    # Optional FSGS components.  These are enabled by the unified
    # corgs_fsgs method and remain off for a plain CoR-GS run.
    parser.add_argument("--fsgs_unpool", action="store_true", default=False)
    parser.add_argument("--fsgs_unpool_until", type=int, default=2000)
    parser.add_argument("--fsgs_unpool_n", type=int, default=3)
    parser.add_argument("--fsgs_depth", action="store_true", default=False)
    parser.add_argument("--fsgs_depth_pseudo", action="store_true", default=False)
    parser.add_argument("--fsgs_depth_weight", type=float, default=0.05)
    parser.add_argument("--fsgs_depth_pseudo_weight", type=float, default=0.5)
    parser.add_argument("--fsgs_depth_pseudo_interval", type=int, default=10)
    parser.add_argument("--fsgs_depth_start", type=int, default=2000)
    parser.add_argument("--fsgs_depth_end", type=int, default=9500)
    parser.add_argument("--fsgs_depth_final_weight", type=float, default=0.001)
