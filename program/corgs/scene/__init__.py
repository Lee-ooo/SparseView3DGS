    ...
    def __init__(self, args : ModelParams, gaussians : GaussianModel, load_iteration=None, shuffle=True, resolution_scales=[1.0]):
        ...
        if os.path.exists(os.path.join(args.source_path, "sparse")):
            if args.source_path.find('llff') != -1:
                ...
                # Generic COLMAP scene (for example, a user-created dataset
                # whose directory name is not one of CoR-GS's benchmarks).
                print("############ load generic COLMAP scene ############")
                scene_info = sceneLoadTypeCallbacks["Colmap"](args.source_path, args.images, args.eval, args.n_views, rand_pcd=args.rand_pcd)
        ...
        for resolution_scale in resolution_scales:
            ...
            pseudo_cams = []
            if args.source_path.find('llff') != -1:
                pseudo_poses = generate_random_poses_llff(
                    self.train_cameras[resolution_scale], n_frames=args.pseudo_views
                )
            elif args.source_path.find('mipnerf360') != -1:
                ...
                # Use the 360-style ellipse for arbitrary COLMAP captures.
                # CoR-GS needs pseudo cameras for its view-consistency loss.
                pseudo_poses = generate_random_poses_360(
                    self.train_cameras[resolution_scale],
                    n_frames=max(int(getattr(args, "pseudo_views", 256)), 0),
                )
