...
class GaussianModel:
    ...
    def restore(self, model_args, training_args):
        ...
        self.denom = denom
        # Confidence is not part of the legacy checkpoint tuple, but the
        # confidence-aware rasterizer still needs one value per Gaussian.
        self.confidence = torch.ones_like(self._opacity, device=self._opacity.device)
        if self.max_radii2D.numel() != self.get_xyz.shape[0]:
    ...
    def densify_and_prune(self, max_grad, min_opacity, extent, max_screen_size, iter):
        ...
        if (getattr(self.args, "fsgs_unpool", False)
                and iter < getattr(self.args, "fsgs_unpool_until", 2000)):
            self.proximity(extent, N=getattr(self.args, "fsgs_unpool_n", 3))
