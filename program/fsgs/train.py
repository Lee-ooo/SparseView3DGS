...
def training(dataset, opt, pipe, args):
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        if iteration % args.sample_pseudo_interval == 0 and iteration > args.start_sample_pseudo and iteration < args.end_sample_pseudo:
            if not pseudo_stack:
                # A dataset without held-out/test cameras has no pseudo cameras.
                # ``Scene.getPseudoCameras`` returns [None] in that case, so
                # filter it before sampling instead of allowing an empty-stack
                # randint failure at the first pseudo-sampling iteration.
                pseudo_stack = [cam for cam in scene.getPseudoCameras().copy() if cam is not None]
            if not pseudo_stack:
                pseudo_stack = None
            else:
