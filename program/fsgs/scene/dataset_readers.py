...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder, path, rgb_mapping):
    cam_infos = []
    rgb_by_name = {
        os.path.normcase(os.path.basename(rgb_path)): rgb_path
    ...
    for idx, key in enumerate(sorted(cam_extrinsics.keys())):
        ...
        if os.path.exists(poses_bounds_path):
            ...
            # Standard COLMAP exports do not contain LLFF's poses_bounds.npy.
            bounds = np.array([0.1, 10.0], dtype=np.float32)
        ...
        rgb_path = rgb_by_name.get(os.path.normcase(image_basename))
        if rgb_path is None:
            raise RuntimeError(
                f"COLMAP image is missing from the dataset images directory: "
                f"{image_basename} (found {len(rgb_mapping)} image files)"
            )
...
def fetchPly(path):
    ...
    # COLMAP's native PLY exporter omits normals.  FSGS only needs them as
    # placeholders for the initial Gaussian cloud, so use zero normals when
...
def readColmapSceneInfo(path, images, eval, n_views=0, llffhold=8):
    # Native FSGS datasets use <n>_views/dense/fused.ply, while the unified
    # COLMAP preparation pipeline provides sparse/0/points3D.ply.  Accept both
    # layouts so FSGS can train directly from the project's prepared dataset.
    ply_candidates = [
        os.path.join(path, str(n_views) + "_views", "dense", "fused.ply"),
        os.path.join(path, "dense", "fused.ply"),
        os.path.join(path, "sparse", "0", "points3D.ply"),
    ]
    ply_path = next((candidate for candidate in ply_candidates if os.path.isfile(candidate)), None)
    if ply_path is None:
        raise FileNotFoundError(
            "No point cloud found. Checked: " + "; ".join(ply_candidates)
        )
